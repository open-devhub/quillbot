...
async function getLanguages(): Promise<Judge0Language[]> {
  ...
  const data = (await res.json()) as Judge0Language[];
...
async function runCode(lang: string, code: string): Promise<RunCodeResult> {
  try {
...
export default {
  ...
  callback: async (client: Client, message: Message, args: string[]) => {
    ...
    if (codeBlockMatch) {
      const [, langMatch, codeMatch] = codeBlockMatch;
      lang = langMatch ?? "";
      code = (codeMatch ?? "").trim();
    } else if (linkMatch) {
      ...
      try {
        ...
        const [, fetchedLang, fetchedCode] = fetchedCodeBlock;
        lang = fetchedLang ?? "";
      ...
      if (output.stderr) {
        ...
        collector.on("collect", async (interaction: ButtonInteraction) => {
          ...
          try {
            ...
            const fixText =
              fixCompletion.choices?.[0]?.message?.content?.slice(0, 1900) ??
              "";
