...
function buildTree(paths: string[], maxDepth = 2): Record<string, any> {
...
function formatTree(obj: Record<string, unknown>, prefix = ""): string {
  ...
  entries.forEach((key, index) => {
    ...
    const isFolder =
      Object.keys(obj[key] as Record<string, unknown>).length > 0;
    const icon = isFolder ? "📁 " : "📄 ";
    ...
    if (isFolder) {
      ...
      result += formatTree(obj[key] as Record<string, unknown>, newPrefix);
    }
  });
...
export default {
  ...
  callback: async (client: Client, message: Message, args: string[]) => {
    ...
    try {
      ...
      const repoData = parseRepo(repoLink);
      if (!repoData) throw new Error("Invalid repo URL");
